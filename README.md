...
- `argocd/applicationsets/`: ArgoCD ApplicationSet 定義
...
develop 用の `enqueue` は `manifest/enqueue-app/values/develop.yaml` で `http` モードにしているため、`make install-enqueue` の時点で HTTP サーバーとして起動します。

`enqueue-app` の deployment を一時停止したい場合は `make enqueue-scale-zero`、再開したい場合は `make enqueue-scale-one` を使います。

HTTP モードを明示的に上書きして `enqueue-app` をデプロイしたい場合は、次を使います。
...
kubectl get jobs
kubectl logs deploy/enqueue-app
kubectl get deploy elasticmq postgresql
kubectl get svc enqueue-app
kubectl port-forward svc/enqueue-app 18080:8080
...
shared / app を含めた全 Helm chart は `manifest/` 配下にあります。app chart の開発用 override は `manifest/enqueue-app/values/develop.yaml` と `manifest/dequeue-app/values/develop.yaml` に置いています。ArgoCD でも `enqueue-app` / `dequeue-app` はこの個別 chart と develop values を直接参照します。
...
4. `argocd/applicationsets/env-bundle.yaml` の `repoURL` を、ArgoCD から到達できる Git remote に置き換えます。

このリポジトリには現時点で Git remote が設定されていないため、ApplicationSet 定義内の `repoURL` はプレースホルダーです。置き換え前に apply すると ArgoCD は同期できません。
...
ApplicationSet とそこで生成される Application は `sample-applicationset` namespace に作られます。ArgoCD 本体は `argocd` namespace のままです。生成される Application は `keda-operator`、`infra-core`、`enqueue-app`、`dequeue-app` の 4 件です。したがって、このコマンドは ArgoCD 管理用クラスタを指す kubecontext で実行してください。
...
kubectl describe application enqueue-app -n sample-applicationset
kubectl describe application dequeue-app -n sample-applicationset
```
